...
class SuperAdminController extends Controller
{
    ...
    /**
     * Abonnements - Gestion des contrats/formules des entreprises
     */
    public function abonnementsIndex()
    {
        ...
        return view('admin.superadmin.abonnements.index', compact('entreprises'));
    }

    /**
     * Facturation Globale - Vue consolidée de toutes les factures
     */
    public function facturationIndex()
    {
        $factures = \App\Models\Facture::with(['entreprise', 'client', 'contrat'])
            ->orderByDesc('date_emission')
            ->paginate(20);

        $stats = [
            'total_factures' => \App\Models\Facture::count(),
            'montant_total' => \App\Models\Facture::sum('montant_ttc'),
            'montant_paye' => \App\Models\Facture::sum('montant_paye'),
            'montant_restant' => \App\Models\Facture::sum('montant_restant'),
        ];

        return view('admin.superadmin.facturation.index', compact('factures', 'stats'));
    }

    /**
     * Rapports Globaux - Statistiques consolidées
     */
    public function rapportsIndex()
    {
        $stats = [
            ...
            'total_factures' => \App\Models\Facture::count(),
            'chiffre_affaires' => \App\Models\Facture::sum('montant_ttc'),
        ];
    ...
    /**
     * Gestion APK - Versions de l'application mobile
     */
    public function apkIndex()
    {
        return view('admin.superadmin.apk.index');
    }
    ...
    /**
     * Journal d'Activité - Logs système
     */
    public function journalIndex()
    {
        return view('admin.superadmin.journal.index');
    }

    /**
     * Modèles - Modèles de documents
     */
    public function modelesIndex()
    {
        return view('admin.superadmin.modeles.index');
    }
